        ...
        jQuery("#de-selector .dark-mode").on("click", function() {
           var iteration = $(this).data('iteration') || 1;
            ...
            $(this).data('iteration', iteration);
             ...
             // --------------------------------------------------
         // portfolio hover - Pour la partie Vidéo, gère ton business
         // --------------------------------------------------
         jQuery('.overlay-vid').fadeTo(1, 0);
         // gallery hover
         jQuery(".item .picframe").on("mouseenter", function() {
			 var aw = jQuery(this).parent().find(".overlay-vid");
             aw.width(jQuery(this).find("video").css("width"));
             aw.height(jQuery(this).find("video").css("height"));
             aw.stop(true).fadeTo(200, 1);
             var repicheight = jQuery(this).find("video").css("height");
             var renewheight;
             renewheight = (repicheight.substring(0, repicheight.length - 2) / 2) - 10;
             //alert(newheight);
             //jQuery(this).parent().find(".pf_text").stop(true).animate({'margin-top': newheight},200,'easeOutCubic');
             jQuery(this).parent().find(".pf_textvid").css('margin-top', renewheight);
             jQuery(this).parent().find(".pf_textvid").stop(true).animate({
                 'opacity': '1'
             }, 1000, 'easeOutCubic');
             var bw = jQuery(this).find("video").css("width");
             var bh = jQuery(this).find("video").css("height");
             var bw = parseInt(bw, 10);
             var bh = parseInt(bh, 10);
             var $scale = 1;
             //alert(w);
             jQuery(this).find("video").stop(true).animate({
                 width: bw * $scale,
                 height: bh * $scale,
                 'margin-left': -bw * ($scale - 1) / 2,
                 'margin-top': -bh * ($scale - 1) / 2
             }, 400, 'easeOutCubic');
         }).on("mouseleave", function() {
             var renewheight;
             var repicheight = jQuery(this).find("video").css("height");
             renewheight = (repicheight.substring(0, repicheight.length - 2) / 2) - 10;
             //jQuery(this).parent().find(".pf_text").stop(true).animate({'margin-top': renewheight - 30},200,'easeOutCubic');
             jQuery(this).parent().find(".pf_textvid").stop(true).animate({
                 'opacity': '0'
             }, 400, 'easeOutCubic');
             jQuery(this).parent().find(".overlay-vid").stop(true).fadeTo(200, 0);
             jQuery(this).find("video").stop(true).animate({
                 width: '100%',
                 height: '100%',
                 'margin-left': 0,
